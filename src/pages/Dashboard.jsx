...
function SuperAdminBanner() {
  const active = localStorage.getItem("super_admin_impersonating") === "1";
  if (!active) return null;

  function returnToSuperAdmin() {
    const originalToken = localStorage.getItem("super_admin_original_token");
    const originalUser = localStorage.getItem("super_admin_original_user");
    if (originalToken) localStorage.setItem("auth_token", originalToken);
    if (originalUser) localStorage.setItem("auth_user", originalUser);
  ...
  return (
    <div style={{ marginBottom: 16, borderRadius: 20, padding: 16, background: "#111827", color: "white", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
      <div style={{ fontWeight: 900 }}>Modalità Super Admin: stai gestendo questo ristorante con permessi completi.</div>
      <button onClick={returnToSuperAdmin} style={{ border: 0, borderRadius: 14, padding: "10px 14px", background: "white", color: "#111827", fontWeight: 900, cursor: "pointer" }}>
        Torna a Super Admin
      </button>
    </div>
  );
  ...
  if (!restaurantName) {
    return (
      <div style={glowPageStyle}>
        <Navbar />
        <div style={appShellStyle}><div className="app-shell">
          <SuperAdminBanner /><Panel title="Nessun ristorante attivo" subtitle="Accedi dall’area admin per continuare."><Link to="/admin" style={primaryBtn}>Vai all’area admin</Link></Panel></div></div>
      </div>
    );
  }
